...
- **REGRA CRÍTICA — propagar mudanças de fonte de dado/constante/lógica compartilhada pra TODOS os dependentes, na mesma tarefa, nunca faseado.** Registrada jul/2026 depois de 3 incidentes reais na mesma sessão: (a) a descrição da imputação de cadência foi corrigida em `docs/variaveis.txt` mas ficou errada em `CLAUDE.md` (mesma tabela, arquivo diferente) até uma auditoria manual pegar; (b) o relatório HTML tinha AUC-ROC e nome do modelo hardcoded — ficaram errados assim que o modelo foi retreinado; (c) **o mais grave:** quando `scoring_mensal.py` passou a ler dado AO VIVO em vez de `df_model_teste.parquet` (Passo 2, decisão 54), `explicar_cliente.py`/`.ipynb` não foram atualizados juntos — o mesmo cliente aparecia com o sinal de `meses_sem_pedido_pre` em direções opostas dependendo de qual ferramenta se abria, **sem nenhum erro ou aviso** (decisão 56). Checklist obrigatório antes de considerar qualquer mudança concluída:
  1. **Mudou qual arquivo/fonte um script lê** (ex.: trocar `df_model_teste.parquet` por dado ao vivo)? `grep` o nome do arquivo/fonte antigo no repositório inteiro — cada resultado é candidato a precisar da mesma mudança.
  2. **Mudou um número ou fato que aparece em prosa** (AUC, contagem de clientes, nome do modelo oficial, fórmula de imputação)? `grep` o valor antigo em `CLAUDE.md`, `README.md`, `docs/*.txt` — não só no arquivo que motivou a mudança.
  3. **Ferramentas que fazem a mesma coisa em paralelo são pares** — `explicar_cliente.py` ↔ `explicar_cliente.ipynb`; `scoring_mensal.py` ↔ `gerar_relatorio.py`. Mudar uma sem checar a irmã foi a causa mais comum de inconsistência nesta sessão.
  4. **Antes de reportar uma tarefa como concluída**, rodar uma busca rápida pelo termo/arquivo/número central da mudança no repositório inteiro — não só nos arquivos que já sei que editei.
...
56. **BUG — `explicar_cliente.py`/`.ipynb` explicavam com dado desatualizado, corrigido (jul/2026).** Achado pelo usuário: o relatório HTML mostrava o cliente 256 (Hospital Evangélico de Cachoeiro) com `meses_sem_pedido_pre=8` empurrando pra churn, mas o waterfall do `explicar_cliente` mostrava a mesma feature em **azul** (empurrando pra longe do churn) pro mesmo cliente. Não era bug de sinal do SHAP — era **fonte de dado diferente**: `explicar_cliente.*` ainda lia `df_model_teste.parquet` (snapshot congelado em `CUTOFF_TESTE`, onde esse cliente tinha `meses_sem_pedido_pre=1`), enquanto o relatório já usava `features_scoring_atual.parquet` (dado ao vivo do Passo 2, decisão 54, onde o mesmo cliente estava em 8). Mesmo cliente, dois retratos em momentos diferentes, sinais opostos — nenhum dos dois "errado" isoladamente, só inconsistentes entre si. **Corrigido:** `explicar_cliente.py` e `explicar_cliente.ipynb` agora leem `features_scoring_atual.parquet` (com `FileNotFoundError` claro se `scoring_mensal.py` ainda não rodou neste ciclo) — nunca mais `df_model_teste.parquet`. Validado: cliente 256 recalculado bate com o relatório (`p_calibrada=0.522`, `meses_sem_pedido_pre=8` em vermelho, +0.85, maior driver). **Padrão a vigiar:** toda ferramenta de inspeção por cliente deve usar a mesma fonte de dado do pipeline de inferência ativo — nunca o snapshot de treino/teste, que representa outro momento no tempo.
57. **Clientes já `Inativo` no cadastro excluídos automaticamente da lista de alerta (jul/2026).** Pedido do usuário: não faz sentido a equipe de retenção receber alerta de quem já saiu. `scripts/scoring_mensal.py` agora consulta `STATUS` ao vivo (`CONTROLLER.dbo.PBI_Clientes` — só existem os valores `Ativo`/`Inativo` no cadastro hoje) pra todos os clientes scoreados, e filtra `status_sistema == "Inativo"` **antes** do ranking por `valor_em_risco` — não depois. Isso importa: filtrar antes garante que a lista continua com os `N_CAPACIDADE` completos (o próximo cliente ativo da fila ocupa a vaga), em vez de devolver uma lista incompleta. `status_sistema` foi adicionado como coluna em `scores_2026.parquet` (auditável — clientes inativos continuam aparecendo no parquet completo, só nunca em `entra_na_lista`).
    - **Achado relevante:** na 1ª execução com o filtro, **206 dos 500 clientes acima do piso de risco (41%) já estavam Inativo** — validação forte de que o modelo está de fato capturando sinal real de churn, não ruído.
    - **Duplicação corrigida no caminho:** `PISO_RISCO` e `N_CAPACIDADE` estavam hardcoded separadamente em `scoring_mensal.py` e `gerar_relatorio.py` (o segundo usava até uma representação diferente do mesmo valor). Movidos pra `src/config.py`, seguindo a convenção já existente do projeto ("Constantes centralizadas em `src/config.py`") — os dois scripts agora importam de lá.
    - `scripts/gerar_relatorio.py` também atualizado: texto de metodologia menciona a exclusão de inativos e mostra quantos foram excluídos neste ciclo (`n_ja_inativos`, calculado com o mesmo critério — acima do piso E inativo — pra bater com o número que o `scoring_mensal.py` imprime).
58. **`historico_insuficiente` desmembrado em duas ressalvas distintas (jul/2026) — achado do usuário comparando os clientes 256 e 1641.** O flag único conflava dois casos bem diferentes sob a mesma "†": (a) cliente novo/esparso, onde o **score** tem pouco comportamento pra se basear (`sem_historico_cadencia` ou `sem_historico_itens`); (b) cliente com histórico normal que simplesmente parou de comprar há tempo suficiente pra a janela de **receita anual** (últimos 12 meses) não pegar 12 meses reais — a receita fica extrapolada, mas o score continua bem fundamentado (às vezes é o próprio silêncio que empurra o score pra cima). Concretamente: cliente 256 (29 meses de histórico muito regular, comprava quase todo mês) tinha `sem_historico_cadencia=0` e `sem_historico_itens=0` — nenhum dos dois motivos de score incerto — mas caía na mesma bandeira genérica de um cliente com 1 mês de vida (cliente 1641) só porque a receita dos últimos 12 meses tinha só 3 meses reais (parou de comprar em out/2025). **Corrigido:**
